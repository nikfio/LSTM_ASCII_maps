...
if( argc != 4 ) {
	cout << "Not valid Scenarios folder" 
	     << "./TestRRT path-to-Scenarios-folder [num maps] " 
...
ScenarioList TestList(TestPaths, TestPaths.size(), 500);
...
cout << TestList.scenario_archive.size() << endl;
for( int k = 0; k < num_scen; k++) {
	for(int i = 0; i < num_paths_per_scen; i++) {

		RRT Agent(TestList.scenario_archive[k], directionMode);
		...
		Agent.search_path_goal_biased(cost, PathFound, par_index, elapsed_time);
