...
//				<<  " next dir: " << nextPix << "  target: " << target <<  endl;
			...
			for(int i = 0; i < dist_state.size(); i++) {
				store_data[i] = ceil(dist_state[i]);
			}

			store_data[dist_state.size()] = ceil(getModDistance(current, target));
...
//			string test_state = "test_point";
//			printMap_point(map, current, test_state, 'N');
//			
//			getchar();
